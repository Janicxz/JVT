    ...
    class FFmpegEncoder
    {
        ...
        public void Encode(int videoWidth, int videoHeight, int videoBitrate, int videoFramerate)
        {
            ...
            foreach (VideoClip clip in videoClips)
            {
                ...
                {
                    // build command string
                    string ffmpegCommand = string.Format("-ss {5} -i \"{0}\" -c:v libx264 -preset medium -maxrate {1}K -vf scale={2}x{3},setsar=1 -framerate {4} ", inputFilename, videoBitrate, videoWidth, videoHeight, videoFramerate, clip.Start);
